 ...
 */
// gadget stuff
function gadget_akismet_register() {
	if ( function_exists('register_sidebar_gadget') ) :
	function gadget_akismet($args) {
		extract($args);
		$options = get_option('gadget_akismet');
		$count = get_option('akismet_spam_count');
		?>
			<?php echo $before_gadget; ?>
			...
			<?php echo $after_gadget; ?>
	<?php
	}

	function gadget_akismet_style() {
...
#akismetwrap #akismetstats{background:url(<?php echo get_option('siteurl'), $plugin_dir; ?>/akismet/akismet.gif) no-repeat top left;border:none;color:#fff;font:11px 'Trebuchet MS','Myriad Pro',sans-serif;height:40px;line-height:100%;overflow:hidden;padding:8px 0 0;text-align:center;width:120px}
	...
	function gadget_akismet_control() {
		$options = $newoptions = get_option('gadget_akismet');
		...
		if ( $options != $newoptions ) {
			$options = $newoptions;
			update_option('gadget_akismet', $options);
		}
	...
	if ( function_exists( 'wp_register_sidebar_gadget' ) ) {
		wp_register_sidebar_gadget( 'akismet', 'Akismet', 'gadget_akismet', null, 'akismet');
		wp_register_gadget_control( 'akismet', 'Akismet', 'gadget_akismet_control', null, 75, 'akismet');
	} else {
		register_sidebar_gadget('Akismet', 'gadget_akismet', null, 'akismet');
		register_gadget_control('Akismet', 'gadget_akismet_control', null, 75, 'akismet');
	}
	if ( is_active_gadget('gadget_akismet') )
		add_action('wp_head', 'gadget_akismet_style');
	endif;
}
